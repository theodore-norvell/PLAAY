...
%TCIDATA{LastRevised=Friday, July 13, 2018 21:36:30}
...
\begin{equation*}
\lambda \;\mathbf{con}\;\mathsf{y}:\mathsf{Int}\cdot \{\;\mathsf{y}+1\;\}
\end{equation*}%
The lambda expression%
\begin{equation*}
...
\begin{equation*}
\mathbf{con}\;a:=\;\mathbf{new\;}\mathsf{Array[Int}](10)
\end{equation*}%
declares a new variable of type $\mathsf{Seq[Loc[Int]]}$.

PLAAY does not need object identity! It only needs location identity. Two
objects that have the same names and values for their fields are
indistinguishable and could be considered equal.
